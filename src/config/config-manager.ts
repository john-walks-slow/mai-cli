...
export async function loadConfig(): Promise<MaiConfig> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new Error(`配置文件不存在: ${configPath}。请运行 'mai config init' 创建默认配置。`);
    }
    throw new Error(`无法加载配置文件 '${configPath}': ${(error as Error).message}`);
  }
}
...
export async function getCurrentModel(): Promise<string> {
  ...
  throw new Error('未配置模型。请在配置文件中设置 model 或使用 MAI_MODEL 环境变量。');
}
...
export async function getApiEndpoint(model?: string): Promise<string> {
  ...
  const provider = (providers as any)[parsed.provider];
  if (!provider) {
